...
{
    public class UpdateExcelServiceInternas : IUpdateExcelServiceInternas
    {
        private readonly DbSmpContext _context;
        private readonly ILogger<UpdateExcelServiceInternas> _logger;
        private const int MaxRetryAttempts = 3;
        private const int RetryDelayMilliseconds = 2000;

        public UpdateExcelServiceInternas(DbSmpContext dbContext, ILogger<UpdateExcelServiceInternas> logger)
        {
        ...
        public string SearchIntervalCuadInter(DateTime dateStart, DateTime dateEnd)
        {
            try
            {
                // Limpiar datos existentes del Excel desde la fila 2
                var filePath = Path.Combine(Directory.GetCurrentDirectory(), "xlsx", "CALCULO_DE_PAGO_A_CUADRILLA INTERNAS.xlsx");
                _logger.LogInformation("Intentando abrir archivo Excel en SearchIntervalCuadInter: {FilePath}", filePath);

                if (!File.Exists(filePath))
                {
                    _logger.LogError("Archivo no encontrado en SearchIntervalCuadInter: {FilePath}", filePath);
                ...
                // Guardar cambios después de limpiar
                workbook.Save();

                // Obtener valores de TD.Concepts desde la base de datos
                var conceptsFromDb = _context.Set<Concepts>()
                    .Where(c => c.Active)
                    .ToDictionary(c => c.Id, c => c.InternalTeamValue ?? 0);

                _logger.LogInformation("Se obtuvieron {Count} conceptos activos de la base de datos", conceptsFromDb.Count);
                ...
                foreach (var grupoOT in conceptosPorOT)
                {
                    ...
                    foreach (var concepto in conceptosDeEstaOT)
                    {
                        ...
                        var excelData = new ExcelDataRequestCuadInter
                        {
                            ...
                            ReconexionMuro = concepto.NombreConcepto == "RECONEXIÓN EN MURO" ? data : null,
                            CorteTomaTierra = concepto.NombreConcepto == "CORTE EN TIERRA" ? data : null,
                            CorteTomaMuro = concepto.NombreConcepto == "CORTE DE TOMA EN MURO" ? data : null,
                            CorteBanqueta = concepto.NombreConcepto == "CORTE EN BANQUETA" ? data : null,
                            CorteAsfaltoPredio = concepto.NombreConcepto == "CORTE EN ASFALTO (EN DIRECCION AL PREDIO)" ? data : null,
                        ...
                        _logger.LogInformation( "INST. VÁLVULA 1/2\\\"");
                        ...
                        // Enviar al método externo
                        var resultadoModificacion = ModificarOInsertarCuadInter(excelData, null);
                        contador++;
...
// Encuentra la columna correcta
                ...
                ModificarOInsertarCuadInter(new ExcelDataRequestCuadInter
                {
                    ...
                    CorteAsfaltoPredio = $"=SUM(AL2:AL{contador})",
                ...
                // Insertar valores de tarifas desde TD.Concepts
                ModificarOInsertarCuadInter(new ExcelDataRequestCuadInter
                {
                    ResaneDeBanqueta = conceptsFromDb.ContainsKey(1) ? conceptsFromDb[1] : 0,
                    FugaEnMedidor = conceptsFromDb.ContainsKey(2) ? conceptsFromDb[2] : 0,
                    FugaEnBanqueta = conceptsFromDb.ContainsKey(3) ? conceptsFromDb[3] : 0,
                    InstMedidor12112Piso = conceptsFromDb.ContainsKey(4) ? conceptsFromDb[4] : 0,
                    InstMedidor12112Arco = conceptsFromDb.ContainsKey(5) ? conceptsFromDb[5] : 0,
                    InstMedidor2Caja = conceptsFromDb.ContainsKey(6) ? (conceptsFromDb[6] != 0 ? conceptsFromDb[6] : (decimal?)null) : null,
                    InstMedidor3Caja = conceptsFromDb.ContainsKey(7) ? (conceptsFromDb[7] != 0 ? conceptsFromDb[7] : (decimal?)null) : null,
                    InstMedidor4Caja = conceptsFromDb.ContainsKey(8) ? (conceptsFromDb[8] != 0 ? conceptsFromDb[8] : (decimal?)null) : null,
                    InstValvula12 = conceptsFromDb.ContainsKey(9) ? conceptsFromDb[9] : 0,
                    CambioMedidor12112 = conceptsFromDb.ContainsKey(10) ? conceptsFromDb[10] : 0,
                    ReconexionAsfalto = conceptsFromDb.ContainsKey(11) ? conceptsFromDb[11] : 0,
                    CorteAsfaltoRed = conceptsFromDb.ContainsKey(12) ? conceptsFromDb[12] : 0,
                    CorteExtMedidorMadera = conceptsFromDb.ContainsKey(13) ? conceptsFromDb[13] : 0,
                    ReconexionBanqueta = conceptsFromDb.ContainsKey(14) ? conceptsFromDb[14] : 0,
                    ReconexionTierra = conceptsFromDb.ContainsKey(15) ? conceptsFromDb[15] : 0,
                    ReconexionMuro = conceptsFromDb.ContainsKey(16) ? conceptsFromDb[16] : 0,
                    CorteTomaTierra = conceptsFromDb.ContainsKey(17) ? conceptsFromDb[17] : 0,
                    CorteTomaMuro = conceptsFromDb.ContainsKey(18) ? conceptsFromDb[18] : 0,
                    CorteBanqueta = conceptsFromDb.ContainsKey(19) ? conceptsFromDb[19] : 0,
                    CorteAsfaltoPredio = conceptsFromDb.ContainsKey(29) ? conceptsFromDb[29] : 0,
                    ReconexionDrenaje = conceptsFromDb.ContainsKey(20) ? conceptsFromDb[20] : 0,
                    CorteDrenajeTapon = conceptsFromDb.ContainsKey(21) ? conceptsFromDb[21] : 0,
                    CorteDrenajeRegistro = conceptsFromDb.ContainsKey(22) ? (conceptsFromDb[22] != 0 ? conceptsFromDb[22] : (decimal?)null) : null,
                    GastoVisitaObra = conceptsFromDb.ContainsKey(23) ? conceptsFromDb[23] : 0,
                    SondeoTierra = conceptsFromDb.ContainsKey(24) ? conceptsFromDb[24] : 0,
                    SondeoBanqueta = conceptsFromDb.ContainsKey(25) ? conceptsFromDb[25] : 0,
                    SondeoPavimento = conceptsFromDb.ContainsKey(26) ? conceptsFromDb[26] : 0,
                    RetiroTapones = conceptsFromDb.ContainsKey(27) ? conceptsFromDb[27] : 0
                }, null);
                ...
                ModificarOInsertarCuadInter(new ExcelDataRequestCuadInter
                {
                    ...
                    CorteAsfaltoPredio = $"=AL{contador+1}*AL{contador+2}",
                ...
                ModificarOInsertarCuadInter(new ExcelDataRequestCuadInter
                {
                    ...
                    CorteAsfaltoPredio = $"=AK{contador+4}+AL{contador+3}",
                ...
                ModificarOInsertarCuadInterUnido(new ExcelDataRequestCuadInter
                {
                    SondeoBanqueta = "PORCENTAJE DE RETENCION POR APROVACION DE GMAS"
                }, null);
                ModificarOInsertarCuadInterUnido(new ExcelDataRequestCuadInter
                {
                    SondeoBanqueta = "TOTAL",
                    RetiroTapones = $"=AT{contador+4}+AT{contador+5}"
                }, null);
        ...
        public string ModificarOInsertarCuadInter(ExcelDataRequestCuadInter data, IXLWorksheet hoja = null)
        {
            ...
            ActualizarFilaCuadInter(hoja, nuevaFila, columnas, data);
            workbook.Save();

            return $"Se añadió nueva fila en: {nuevaFila}";
        }
        
        public string ModificarOInsertarCuadInterUnido(ExcelDataRequestCuadInter data, IXLWorksheet hoja = null)
        {
            var disposeWorkbook = false;
            XLWorkbook workbook = null;
            if (hoja == null)
            ...
            {
            var rango1 = hoja.Range($"A2:AT{nuevaFila-6}");
            ...
            var rango2 = hoja.Range($"S{nuevaFila-5}:AT{nuevaFila-2}");
            ...
            var rango3 = hoja.Range($"AR{nuevaFila-1}:AS{nuevaFila}");
            ...
            var rango4 = hoja.Range($"AT{nuevaFila-1}:AT{nuevaFila}");
        ...
        private void ActualizarFilaCuadInter(IXLWorksheet hoja, int fila, Dictionary<string, int> columnas, ExcelDataRequestCuadInter data)
        {
            ...
            // CorteAsfaltoPredio
            if (data.CorteAsfaltoPredio is string formula12 && formula12.StartsWith("="))
            {
                hoja.Cell(fila, columnas["CORTE_ASFALTO_PREDIO"]).FormulaA1 = formula12;
            }
            else if (data.CorteAsfaltoPredio is decimal dec12)
            {
                hoja.Cell(fila, columnas["CORTE_ASFALTO_PREDIO"]).Value = dec12;
            }
            else if (data.CorteAsfaltoPredio != null)
            {
                hoja.Cell(fila, columnas["CORTE_ASFALTO_PREDIO"]).Value = data.CorteAsfaltoPredio.ToString();
            }
            else
            {
                hoja.Cell(fila, columnas["CORTE_ASFALTO_PREDIO"]).Clear();
            }
        ...
        public byte[] GetExcelFileCuadInter()
        {
            try
            {
                var filePath = Path.Combine(Directory.GetCurrentDirectory(), "xlsx", "CALCULO_DE_PAGO_A_CUADRILLA INTERNAS.xlsx");
    ...
    public interface IUpdateExcelServiceInternas
    {
        string SearchIntervalCuadInter(DateTime dateStart, DateTime dateEnd);
        string ModificarOInsertarCuadInter(ExcelDataRequestCuadInter data, IXLWorksheet hoja = null);
        byte[] GetExcelFileCuadInter();
    }
}