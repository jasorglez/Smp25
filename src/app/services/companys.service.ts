 ...
 ------------------------------------------------------------------------------------*/
  ...
  getDataCompanys(clave: string): Observable<Icompany | null> {
    if (clave !== '') {
      return this.http
        .get<any>(
          `${environment.urlFirebase}companys.json?orderBy="$key"&equalTo="${clave}"`
        )
        .pipe(
          map((data) => {
  ...
  async addpermiscompany(
    ...
    try {
      await this.http
        .post(`${environment.urlFirebase}permissionsxcompanys.json`, data)
        .toPromise();
    } catch (error) {
      ...
      return this.http.get(apiUrl).pipe(
        map((permissions: any) => {
          return Object.values(permissions).filter((permission: any) => {
            return (
              permission.id_company === id_company && permission.email === mail
            );
          });
        })
      );
  ...
  postData(data: Icompany, token: any) {
    try {
      return this.http.post(
        `${environment.urlFirebase}companys.json?auth=${token}`,
        data
      );
    } catch (error) {
  ...
  patchData(id: string, data: object, token: any) {
    try {
      return this.http.patch(
        `${environment.urlFirebase}companys/${id}.json?auth=${token}`,
        data
      );
    } catch (error) {
      //  alerts.basicAlert("error", `Error save Users${error}`, "error")
  ...
  Companys(): Observable<any> {
    try {
      const apiUrl = `${environment.urlAzure}api/Companys/id`;
      //  alert(apiUrl)
      return this.http.get(apiUrl);
    } catch (error) {
  ...
  getProjectxCompany(prio: number, idcomp: number): Observable<any> {
    try {
      const apiUrl = `${environment.urlAzure}api/Project/company?priority=${prio}&comp=${idcomp}`;
      //  alert(apiUrl)
      return this.http.get(apiUrl);
    } catch (error) {
