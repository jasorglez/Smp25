...
export class ReceiptsDelisonService {
            ...
            body:
              this.detailedReq.type == 'OC'
                ...
                : [
                    ...
                    ...this.reqItems.map((item, index) => [
                      { text: (index + 1).toString(), style: 'tableCell' },
                      // Si idSupplie es 0, usar numArticle, si no, usar code
                      { text: item.idSupplie === 0 ? (item.numArticle || '') : (item.code || ''), style: 'tableCell' },
                      {
                        text: item.quantity.toString(),
                        style: 'tableCell',
                        alignment: 'right',
                      },
                      // Si idSupplie es 0, usar nameArticle, si no, usar description
                      { text: item.idSupplie === 0 ? (item.nameArticle || item.description || '') : (item.description || ''), style: 'tableCell' },
                      ...(!this.isInOut
                        ? [
                            {
