...
export function normalizeEstadoMexicoName(estado: string): string {
  const cleanEstado = String(estado ?? '').trim();
  if (!cleanEstado) return '';

  const alias = ESTADO_ALIASES[cleanEstado];
  if (alias) return alias;

  const normalizedEstado = normalizeText(cleanEstado);
  const directMatch = ESTADOS_MEXICO.find((item) => normalizeText(item) === normalizedEstado);
  if (directMatch) return directMatch;

  const aliasMatch = Object.entries(ESTADO_ALIASES).find(([key]) => normalizeText(key) === normalizedEstado);
  return aliasMatch?.[1] ?? cleanEstado;
}

export function getMunicipiosByEstado(estado: string): string[] {
  const estadoNormalizado = normalizeEstadoMexicoName(estado);
  return MUNICIPIOS_MEXICO[estadoNormalizado] ?? [];
}

export function normalizeMunicipioMexicoName(estado: string, municipio: string): string {
  const cleanMunicipio = String(municipio ?? '').trim();
  if (!cleanMunicipio) return '';

  const municipios = getMunicipiosByEstado(estado);
  const normalizedMunicipio = normalizeText(cleanMunicipio);
  const match = municipios.find((item) => normalizeText(item) === normalizedMunicipio);
  return match ?? cleanMunicipio;
}
