...
import { CustomersPaymentsComponent } from '../customers/customers-payments.component';
import { CustomersBillingComponent } from '../customers/customers-billing.component';
...
@Component({
  selector: 'app-palacio-contribuyente',
  standalone: true,
  imports: [
    RouterModule,
    DomainsModule,
  ...
  templateUrl: './palacio-contribuyente.component.html',
  styleUrls: ['./palacio-contribuyente.component.scss'],
})
export class PalacioContribuyenteComponent implements CanComponentDeactivate {
  ...
  obtenerDatos() {
    this.trackingService.addLog(this.trackingService.getnameComp(), `Mostrar Listado de Contribuyentes - Palacio Municipal`, 'Menu Administracion - Palacio Municipal',
    ...
    return new Promise((resolve) => {
      this.customerService
        .getCustomersxPalace(this.idRoot)
        .subscribe({
          next: (data: any) => {
            this.rowData = data;
            resolve(true);
  ...
  async saveChanges() {
    const isValid = this.rowData.every(
      (item) => item.nameContact
    );
    if (!isValid) {
      alerts.basicAlert(
        'Añadir entrada',
        'Debe llenar el nombre del contribuyente antes de guardar.',
        'error'
      );
      return;
    }
