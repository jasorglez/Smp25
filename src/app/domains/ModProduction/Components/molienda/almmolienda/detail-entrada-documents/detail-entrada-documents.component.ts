...
@Component({
  selector: 'app-detail-entrada-documents',
  standalone: true,
...
export class DetailEntradaDocumentsComponent {
  ...
  async saveChanges() {
    ...
    const adds = newRows.map((r) => this.intandoutDocumentsService.addIntandoutDocuments(this.cleanRow(r)));
    const updates = modifiedRows.map((r) => this.intandoutDocumentsService.updateIntandoutDocuments(r.id, this.cleanRow(r)));
    try {
  ...
  deleteEntry() {
    ...
    this.intandoutDocumentsService.deleteIntandoutDocuments(data.id)
      .pipe(catchError(() => {
