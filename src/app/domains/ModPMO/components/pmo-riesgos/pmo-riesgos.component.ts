...
export class PmoRiesgosComponent {
  ...
  loadConceptsAndRisks(): void {
    ...
    this.workprogramsService.getWorkPrograms(sourceId, this.typeWorkProgram).subscribe({
      ...
      error: () => { this.conceptOptions = []; this.loadRisks(); }
    });
  }
  ...
  onConceptChanged(): void {
    const selected = this.conceptOptions.find(c => c.id === Number(this.form.conceptId));
    this.form.conceptCode = selected?.code ?? '';
    this.form.conceptName = selected?.name ?? '';
  }
  ...
  saveRisk(): void {
    if (!this.idProject) { alerts.basicAlert('Aviso', 'Selecciona primero un proyecto desde el sidebar.', 'warning'); return; }
  ...
  saveFollowUp(): void {
    ...
    request$.subscribe({
      ...
      error: () => { this.isSavingFollowUp = false; alerts.basicAlert('Error', `No fue posible ${this.followUpForm.id ? 'actualizar' : 'registrar'} el seguimiento.`, 'error'); }
    });
  }

  async deleteFollowUp(followUp: RiskFollowUp): Promise<void> {
    if (!this.form.id) return;
    const confirm = await alerts.confirmAlert('¿Eliminar seguimiento?', 'Se quitará del historial del riesgo PMO.', 'question', 'Eliminar');
    if (!confirm.isConfirmed) return;

    this.riskmatrixService.deletePmoRiskFollowUp(this.form.id, followUp.id).subscribe({
      next: (risk: any) => { this.replaceRiskRecord(this.mapApiRiskToRecord(risk)); this.followUpForm = this.createEmptyFollowUp(); alerts.basicAlert('Seguimiento', 'Seguimiento eliminado correctamente.', 'success'); },
  ...
  async deleteRisk(): Promise<void> {
    ...
    const confirm = await alerts.confirmAlert('¿Eliminar riesgo?', 'El riesgo quedará inactivo en PMO.', 'question', 'Eliminar');
    if (!confirm.isConfirmed) return;
  ...
  private loadRisks(): void {
    this.riskmatrixService.getPmoRisks(this.idProject, this.idContract).subscribe({
      next: (data: any[]) => { this.risks = (data || []).map(item => this.mapApiRiskToRecord(item)); this.isLoading = false; },
  ...
  private mapApiRiskToRecord(item: any): RiskRecord {
    ...
    const selectedConcept = this.conceptOptions.find(c => c.id === conceptId);
    return {
      id: item.id ?? null,
      folio: item.folio ?? '',
      title: item.title ?? '',
      ...
      conceptName: item.workProgramDisplay ?? item.workProgramText ?? selectedConcept?.name ?? '',
  ...
  private compareRisks(left: RiskRecord, right: RiskRecord): number {
    ...
    switch (this.sortField) {
      case 'title':     cmp = left.title.localeCompare(right.title); break;
      case 'dueDate':   cmp = this.compareDates(left.dueDate, right.dueDate); break;
      case 'criticality': cmp = left.criticality - right.criticality; break;
      default:          cmp = this.compareDates(left.updatedAt || left.createdAt, right.updatedAt || right.createdAt); break;
    }
  ...
  private calculateCriticality(probability: number, impactTimeDays: number, impactCost: number): number {
    return probability * Math.max(this.getTimeScale(impactTimeDays), this.getCostScale(impactCost));
  }

  private calculateTrafficLight(probability: number, impactTimeDays: number, impactCost: number): RiskRecord['trafficLight'] {
    const c = this.calculateCriticality(probability, impactTimeDays, impactCost);
    if (c >= 16) return 'Crítico';
    if (c >= 11) return 'Alto';
    if (c >= 6)  return 'Medio';
    return 'Bajo';
  }
  ...
  private getTimeScale(days: number): number {
    if (days >= 15) return 5; if (days >= 8) return 4; if (days >= 4) return 3; if (days >= 2) return 2; return 1;
  }

  private getCostScale(amount: number): number {
    if (amount >= 500000) return 5; if (amount >= 200000) return 4; if (amount >= 100000) return 3; if (amount >= 25000) return 2; return 1;
  }
