...
export class DetailCellRendererComparisonComponent implements ICellRendererAngularComp {
  ...
  private async loadData(rowData: any) {
    ...
    try {
      ...
      this.buildColDefs(providers, PROVIDER_STYLES);
    ...
    providers.forEach((prov, i) => {
      ...
      this.colDefs.push({
        ...
        cellRenderer: (params: any) => {
          ...
          if (!data) {
            return '<span style="color:#bbb;font-size:10px;padding:2px 6px;">N/C</span>';
          }
          const isBest = this.isBestPrice(params.node?.data, slot);
          const weight = isBest ? 'bold' : 'normal';
          const color  = isBest ? '#155724' : '#333';
          const check  = isBest ? '✓ ' : '';
        ...
        cellStyle: (params: any) => {
          ...
          const isBest = this.isBestPrice(params.data, slot);
          return isBest
