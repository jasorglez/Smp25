...
import { DimensionService, Dimension } from 'app/services/dimension.service';
import { SignalsService } from 'app/services/signals.service';
import { lastValueFrom } from 'rxjs';

interface DimensionRow extends Dimension {
  __isNew?: boolean;
  __modified?: boolean;
}
...
export class DimensionesComponent implements OnInit {
  ...
  rowData: DimensionRow[] = [];
  private originalRowData: DimensionRow[] = [];
  hasChanges = false;
  saving     = false;
  selectedRow: DimensionRow | null = null;
  ...
  private loadData() {
    this.svc.getByCompany(this.idCompany).subscribe({
      next: (data) => {
        this.rowData = data as DimensionRow[];
        this.originalRowData = JSON.parse(JSON.stringify(this.rowData));
        this.hasChanges = false;
        if (this.gridApi) this.gridApi.setGridOption('rowData', this.rowData);
      },
      error: () => alerts.basicAlert('Error', 'No se pudo cargar dimensiones.', 'error'),
    });
  }

  add() {
    const newRow: DimensionRow = { nombre: '', active: true, idCompany: this.idCompany, __isNew: true, __modified: true };
  ...
  async save() {
    ...
    try {
      ...
      for (const row of toProcess) {
        const payload: Dimension = { id: row.id, nombre: row.nombre, active: row.active, idCompany: this.idCompany };
  ...
  async deleteSelected() {
    ...
    const confirm = await alerts.confirmAlert('Eliminar', '¿Eliminar esta dimensión?', 'warning', 'Sí, eliminar');
    ...
    try {
      ...
      alerts.reqSuccessToast('Eliminado', 'Dimensión eliminada.');
      this.loadData();
      this.selectedRow = null;
    } catch {
      alerts.basicAlert('Error', 'No se pudo eliminar la dimensión.', 'error');
    }
  }
}
