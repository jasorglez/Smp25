...
import { PesoVolumenService, PesoVolumen } from 'app/services/peso-volumen.service';
import { SignalsService } from 'app/services/signals.service';
import { lastValueFrom } from 'rxjs';

interface PesoVolumenRow extends PesoVolumen {
  __isNew?: boolean;
  __modified?: boolean;
}

@Component({
  selector: 'app-peso-volumen',
  standalone: true,
  imports: [CommonModule, AgGridModule],
  template: `
    <div style="padding: 12px; display: flex; flex-direction: column; gap: 10px; height: 420px;">
      <div style="display: flex; align-items: center; justify-content: space-between;">
        <div><strong>Peso / Volumen</strong></div>
...
export class PesoVolumenComponent implements OnInit {
  ...
  private originalRowData: PesoVolumenRow[] = [];
  hasChanges = false;
  saving     = false;
  selectedRow: PesoVolumenRow | null = null;
  ...
  public colDefs: ColDef[] = [
    ...
    { field: 'abreviatura', headerName: 'Abreviatura', width: 130, cellEditor: 'agTextCellEditor' },
    { field: 'nombre', headerName: 'Nombre', flex: 1, minWidth: 160, cellEditor: 'agTextCellEditor' },
    { field: 'tipo', headerName: 'Tipo', width: 130, cellEditor: 'agSelectCellEditor',
      cellEditorParams: { values: ['PESO', 'VOLUMEN'] } },
    { field: 'factorBase', headerName: 'Factor (a kg/L)', width: 150, type: 'numericColumn',
      cellEditor: 'agNumberCellEditor', cellEditorParams: { precision: 8, min: 0 },
      headerTooltip: 'Cuánto vale 1 de esta unidad en la base: kg para PESO, L para VOLUMEN. Ej. ml=0.001, gal=3.78541, lb=0.453592.' },
  ];
  ...
  private loadData() {
    this.svc.getByCompany(this.idCompany).subscribe({
      next: (data) => {
        this.rowData = data as PesoVolumenRow[];
        this.originalRowData = JSON.parse(JSON.stringify(this.rowData));
        this.hasChanges = false;
        if (this.gridApi) this.gridApi.setGridOption('rowData', this.rowData);
      },
      error: () => alerts.basicAlert('Error', 'No se pudo cargar peso/volumen.', 'error'),
    });
  }

  add() {
    const newRow: PesoVolumenRow = { abreviatura: '', nombre: '', active: true, idCompany: this.idCompany, __isNew: true, __modified: true };
  ...
  async save() {
    ...
    try {
      ...
      for (const row of toProcess) {
        const payload: PesoVolumen = {
          id: row.id, abreviatura: row.abreviatura, nombre: row.nombre,
          tipo: row.tipo ?? null, factorBase: row.factorBase ?? null,
