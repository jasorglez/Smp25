...
export class BitacoraVideosComponent {
  ...
  columnDefs: ColDef[] = [
    ...
    { field: 'imageUrl', headerName: 'URL Video', editable: true, width: 250 },
  ...
  private loadData() {
    ...
    this.logbookService.getInfoByReporte(this.reportData.id, 'Video').subscribe({
      next: (resp: any) => {
  ...
  saveChanges() {
    ...
    if (this.context?.componentParent?.updateBitacoraCount) {
      this.context.componentParent.updateBitacoraCount(this.reportData?.id, 'videos', this.rowData.length);
    }
  }
