...
export class BitacoraMaterialComponent {
  ...
  columnDefs: ColDef[] = [
    ...
    { field: 'idResource', headerName: 'ID Material', editable: true, width: 120 },
  ...
  private loadData() {
    ...
    this.logbookService.getInfoByReporte(this.reportData.id, 'MATERIAL').subscribe({
      next: (resp: any) => {
  ...
  saveChanges() {
    ...
    if (this.context?.componentParent?.updateBitacoraCount) {
      this.context.componentParent.updateBitacoraCount(this.reportData?.id, 'material', this.rowData.length);
    }
  }
