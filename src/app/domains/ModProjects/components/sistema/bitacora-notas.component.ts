...
export class BitacoraNotasComponent {
  ...
  columnDefs: ColDef[] = [
    ...
    { field: 'typeNote', headerName: 'Tipo', editable: true, width: 100 },
    { field: 'orden', headerName: 'Orden', editable: true, width: 80, type: 'numericColumn' },
  ...
  private loadData() {
    ...
    this.logbookService.getInfoByReporte(this.reportData.id, 'NOTE').subscribe({
      next: (resp: any) => {
  ...
  addRow() {
    this.rowData = [{
      ...
      typeNote: 'NOTA', orden: 1, description: '', active: true, __isNew: true, __modified: false
    }, ...this.rowData];
  ...
  saveChanges() {
    ...
    if (this.context?.componentParent?.updateBitacoraCount) {
      this.context.componentParent.updateBitacoraCount(this.reportData?.id, 'notas', this.rowData.length);
    }
  }
