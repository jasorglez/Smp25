...
export class BitacoraConceptosComponent {
  ...
  columnDefs: ColDef[] = [
    ...
    { field: 'idResource', headerName: 'ID Recurso', editable: true, width: 120 },
    { field: 'quantity', headerName: 'Cantidad', editable: true, width: 100, type: 'numericColumn' },
    { field: 'price', headerName: 'Precio', editable: true, width: 100, type: 'numericColumn' },
    { field: 'total', headerName: 'Total', editable: false, width: 120, type: 'numericColumn' },
  ...
  private loadData() {
    ...
    this.logbookService.getInfoByReporte(this.reportData.id, 'CONCEPT').subscribe({
      next: (resp: any) => {
  ...
  addRow() {
    this.rowData = [{
      ...
      quantity: 1, price: 0, total: 0, description: '', active: true, __isNew: true, __modified: false
    }, ...this.rowData];
  ...
  saveChanges() {
    ...
    if (this.context?.componentParent?.updateBitacoraCount) {
      this.context.componentParent.updateBitacoraCount(this.reportData?.id, 'conceptos', this.rowData.length);
    }
  }
