    ...
    <div style="padding: 10px; background-color: #f8f9fa;">
      <div style="margin-bottom: 15px; height: 250px;">
        <div style="margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center;">
          ...
          <div>
            <button class="btn btn-sm btn-success me-2" (click)="addBank()" [disabled]="!bankGridApi">
              <i class="bi bi-plus-circle"></i> Agregar
            </button>
            <button class="btn btn-sm btn-primary me-2" (click)="saveBanks()" [disabled]="!hasBankChanges">
              <i class="bi bi-floppy"></i> Guardar
            </button>
            <button class="btn btn-sm btn-danger" (click)="deleteSelectedBank()" [disabled]="!selectedBank">
...
export class DetailCellRendererComponentBanck implements ICellRendererAngularComp {
  ...
  bankColumnDefs = [
    ...
    { field: 'campo4', headerName: 'Clabe', editable: true, flex: 1 },
    { field: 'campo5', headerName: 'Referencia', editable: true, flex: 1 },
    { field: 'campo7', headerName: 'Activo', editable: true, width: 80, cellEditor: 'agCheckboxCellEditor' }
  ];
  ...
  loadBankData() {
    if (this.params && this.params.context && this.params.context.BANK.load) {
  ...
  saveBanks() {
    if (this.params && this.params.context && this.params.context.BANK.save) {
  ...
  deleteSelectedBank() {
    ...
    if (this.params && this.params.context && this.params.context.BANK.delete) {
      this.params.context.BANK.delete({ data: this.selectedBank, api: this.bankGridApi }, () => {
        this.loadBankData();
