    ...
    <div style="padding: 10px; background-color: #f8f9fa;">
      <div style="margin-bottom: 15px; height: 250px;">
        <div style="margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center;">
          <strong>Cuentas por Pagar de: {{ providerName }}</strong>
          <div>
            <button class="btn btn-sm btn-success me-2" (click)="addCuenta()" [disabled]="!cuentaGridApi">
              <i class="bi bi-plus-circle"></i> Agregar
            </button>
            <button class="btn btn-sm btn-primary me-2" (click)="saveCuentas()" [disabled]="!hasCuentaChanges">
              <i class="bi bi-floppy"></i> Guardar
            </button>
            <button class="btn btn-sm btn-danger" (click)="deleteSelectedCuenta()" [disabled]="!selectedCuenta">
...
export class DetailCellRendererComponentCuentas implements ICellRendererAngularComp {
  ...
  cuentaColumnDefs = [
    ...
    { field: 'campo4', headerName: 'Monto', editable: true, flex: 1, valueFormatter: params => `$${Number(params.value).toFixed(2)}` },
    { field: 'campo5', headerName: 'Saldo', editable: true, flex: 1, valueFormatter: params => `$${Number(params.value).toFixed(2)}` },
  ...
  loadCuentaData() {
    if (this.params && this.params.context && this.params.context.CUENTA.load) {
