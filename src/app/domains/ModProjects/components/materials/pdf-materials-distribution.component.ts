...
@Component({
  selector: 'app-pdf-materials-distribution',
  standalone: true,
...
<div *ngIf="mode === 'embed'" style="padding:10px; background:#f8f9fa; border-radius:6px;">
  ...
  <div class="d-flex justify-content-between align-items-center mb-2">
    <div class="d-flex align-items-center gap-2">
      ...
      <span class="fw-bold" style="font-size:.9rem;">Distribución de Materiales</span>
    </div>
...
export class PdfMaterialsDistributionComponent implements OnInit, ICellRendererAngularComp {
  @Input()  idCompany: number;
  @Input()  mode: 'embed' | 'newTab' = 'embed';
  /** 'quantity' = columnas con cantidades distribuidas | 'money' = cantidad × costoMN */
  ...
  get pdfFileName(): string { return 'Materiales_reporte.pdf'; }
  get pdfSubject(): string  { return 'Distribución de Materiales'; }
  ...
  downloadPdf(): void {
    ...
    a.href     = url;
    a.download = `distribucion-materiales-${new Date().toISOString().substring(0,10)}.pdf`;
    a.click();
  ...
  private async generatePdf(): Promise<void> {
    ...
    try {
      ...
      const [rootResp, materialsResp] = await Promise.all([
        safe(this.rootService.getRootbyId(idRoot)),
        safe(this.materialsService.getAllMaterialsxFamilyview(idRoot)),
      ]);
      this.loadingProgress = 20;

      const root      = rootResp as any;
      const materials = (Array.isArray(materialsResp) ? materialsResp : []) as any[];

      if (!materials.length) { this.isLoading = false; return; }
      ...
      await Promise.all(materials.map(async (mat) => {
        ...
        const dists = await safe(this.distributionService.getByReference('MATERIAL', mat.id, idRoot));
        distMap.set(mat.id, Array.isArray(dists) ? dists : []);
        doneCount++;
        this.loadingProgress = 20 + Math.round((doneCount / totalCount) * 45);
      }));

      // Filtrar solo materiales que tienen distribución
      const materialsWithDist = materials.filter(m => (distMap.get(m.id) ?? []).length > 0);
      if (!materialsWithDist.length) { this.isLoading = false; return; }
        ...
        columns: [
            ...
            : { text: '', width: 80 },
          {
            stack: [
              ...
              { text: isMoney ? 'CUANTIFICADO DE EROGACIONES MENSUAL DE MATERIALES' : 'CUANTIFICADO DE UTILIZACIÓN MENSUAL DE MATERIALES', fontSize: 7, color: GRAY, alignment: 'center', margin: [0,1,0,0] },
            ],
            margin: [10,0,10,0],
          },
          logo2B64
      ...
      // ── TABLA ──────────────────────────────────────────────────────────────
      const colWidths: any[] = ['*', 40, 55, 55, ...sortedMonths.map(() => 38)];
      ...
      materialsWithDist.forEach((mat, idx) => {
        const alt    = idx % 2 === 1;
        const costoMN = Number(mat.costoMN ?? 0);
        const dists   = distMap.get(mat.id) ?? [];
        ...
        const totalQty  = dists.reduce((s, d) => s + Number(d.quantity ?? 0), 0);
        const totalAmt  = isMoney ? totalQty * costoMN : totalQty;
        grandTotal += isMoney ? totalAmt : 0;

        const monthCells = sortedMonths.map((ym, mi) => {
          const qty   = distByMonth.get(ym) ?? 0;
          const value = isMoney ? qty * costoMN : qty;
        ...
        const unit = mat.unidadDescription || mat.measure || '';

        tableBody.push([
          dCell(mat.materialDescription || mat.description || '', alt, 'left'),
          dCell(unit, alt, 'center'),
          { text: `$${costoMN.toFixed(2)}`, fontSize: 7, fillColor: alt ? ALT : WHITE, alignment: 'right' },
          { text: isMoney ? `$${totalAmt.toFixed(3)}` : totalQty.toFixed(3), fontSize: 7, bold: true, fillColor: alt ? ALT : WHITE, alignment: 'right' },
          ...monthCells,
        ]);
      });
      ...
      tableBody.push([
        ...
        totalCell(''), totalCell(''),
        totalCell(isMoney ? `$${grandTotal.toFixed(3)}` : ''),
      ...
      this.trackingService.addLog(this.trackingService.getnameComp(), 'Generó PDF distribución materiales', 'Proyectos / Materiales', this.trackingService.getEmail());
      ...
      console.error('Error generando PDF de distribución de materiales:', err);
      this.isLoading = false;
      this.cdr.detectChanges();
    }
  }
